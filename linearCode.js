...
another link.
It  allows  to  create  a  dynamic  array,  that  is  that  it's  an  array  
that  is  going  to adjust its sizebased onhow many elements are inside of it. */

/*There are two types of linked list:
* Singly Linked List: A chain of node objects where there is a start, and each node keeps track
of the one following it.
* Doubly linked list: A chain of node objects where there is a start and a rear. Each node keeps
track of the node before it and the node after it.*/

/*HEAD-->NODE1[VALUE + NEXT(pointsNode2)]-->NODE2[VALUE + NEXT(pointsNode3)]-->NODE3[VALUE + NEXT(null)]<--TAIL*/
// first node is callled 'head node' & last node is called 'tail node'
// implementing singly linked list(moves L-->R) ---(array)node based approach

/*The singly linked list is almost like an array in JavaScript. It tends to mimic a lot of the 
same ideas as the array, exceptthat it does not have empty slots on deletion. Instead, it  
simply  adjusts  the  size  of  the  list  to  match  the  required  size  after deletion.

Instead of just keeping track of what follows it, a node also keeps track of 
what came  before  it. */
// class Node2 {
//   constructor(value, next) {
//     this.value = value;
...
//   }
//   insertFront() {
//     var frontNode = new Node2(); //creating new node
//     var current; //to store a current node
//     if (this.front == null) {
//       //if list is empty add value & make it head/front node
//       this.front = frontNode;
//       /*If the list is not empty then iterate to the end of the list and add an element at
//       the end of the list*/
//     } else {
//       current = this.front;
//       while (current.next) {
...
//     this.size++;
//   }
//   //insert value/elemnt at particular position in the list
//   insertAt(value, index) {
//     if (index < 0 || index > this.size) {
//       throw "Bas kr Bhai! It is out of your range";
//     } else {
//       if (index == 0) {
...
//         var holder = this.front;
//         /*if the index is between 0 or size – 1 we iterate over
//          to the index and add an element at that index*/
//         while (i < index - 1) {
//           holder = holder.next;
//           i++;
//         }
//         var insertNode = new Node2(value, holder.next);
//         holder.next = insertNode;
//         this.size++;
//       }
//     }
//   }
...
//     }
//   }
//   // finds the index of element/values
//   indexOf(element) {
//     var count = 0;
...
// Link.insertAt(50);
// Link.print();
// console.log("is element removed ?" + Link.delete(50));
// Link.print();
// console.log("Index of 40 " + Link.indexOf(40));
// Link.insertAt(60, 2);
// Link.print();
...
// // implementing doubly linked list ---(queue)node based approach
// class Node3 {
...
//   }
//   insertFront() {
//     var frontNode = new Node3(); //creating new node
//     var current; //to store a current node
//     if (this.front == null) {
//       //if list is empty add value & make it head/front node
//       this.front = frontNode;
...
// Get count
console.log(
  `The count of ${ch} is ${astr[ch.charCodeAt(0) - "a".charCodeAt(0)]}`
);
